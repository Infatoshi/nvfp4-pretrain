...
# Optional CUTLASS sm_120 NVFP4 GEMM backend (vs the default cuBLAS torch._scaled_mm).
_USE_CUTLASS = os.environ.get("NVFP4_CUTLASS", "0") == "1"
import sys as _sys
_CG = os.path.join(os.path.dirname(os.path.abspath(__file__)), "cutlass_gemm")
if _CG not in _sys.path:
    _sys.path.insert(0, _CG)


def _gemm_AB(a, bt):
    """A @ B given a=quant(A) [M,K] and bt=quant(B^T) [N,K] NVFP4Tensors -> bf16 [M,N].
    CUTLASS sm_120 path when NVFP4_CUTLASS=1 and dims are 128-aligned; else cuBLAS."""
    if _USE_CUTLASS:
        ...
        if M % 128 == 0 and N % 128 == 0 and K % 128 == 0:
            ...
            return nvfp4_bf16_gemm(a.qdata, a.scale, bt.qdata, bt.scale, M, N, K, alpha)
...
def quant_nvfp4_cuda(t, stochastic=False, rht=False, H=None, signs=None, seed=None):
    ...
    # per-tensor amax = max(|min|,|max|) via a single aminmax pass (no full abs temp,
    # 1 launch instead of abs-elementwise + amax-reduce). Numerically identical.
