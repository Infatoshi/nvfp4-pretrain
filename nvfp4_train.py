...
if os.environ.get("NVFP4_CUDA", "0") == "1":
    # CUDA-backend quant for all three GEMMs; rebinding fp4_matmul also routes the
    # wgrad path (and non-amortized fprop/dgrad) through the fast quant + the optional
    # CUTLASS sm_120 GEMM (NVFP4_CUTLASS=1), not just fp4_mm_preqB.
    from nvfp4_cuda import quant_nvfp4_cuda, fp4_mm_preqB, fp4_matmul_cuda
    # NVFP4_NAIVE=1 keeps the original slow wgrad (pure-torch quant + cuBLAS) for A/B comparison.
    if os.environ.get("NVFP4_NAIVE", "0") != "1":
        fp4_matmul = fp4_matmul_cuda
    # Fused cast-transpose-quant wgrad: kills the x.t().contiguous() transpose copy.
    # On by default with the CUDA backend; NVFP4_FUSED_CT=0 disables for A/B comparison.
    _FUSED_CT = os.environ.get("NVFP4_FUSED_CT", "1") != "0"
    if _FUSED_CT:
        from cast_transpose_quant import wgrad_fp4_matmul_ct
else:
    _FUSED_CT = False
...
class FP4LinearFn(torch.autograd.Function):
    ...
    def backward(ctx, gy):
        ...
        if _FUSED_CT:                       # fused cast-transpose-quant, no .t().contiguous() copy
            dw = wgrad_fp4_matmul_ct(gy2, x2, sr_gy=True, rht=True, H=H, signs=signs)
        else:
